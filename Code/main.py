...
if __name__=="__main__":
    ...
    train_path = os.path.join(_dir, "../Data/train.ft.txt/train.ft.txt")
    ...
    lr_metrics = get_metrics(y_validation, y_pred)
    lr_table = PrettyTable()
    lr_table.field_names = ["Metric", "Value"]
    lr_table.add_row(["Accuracy",  f"{lr_metrics['Accuracy']:.4f}"])
    lr_table.add_row(["Precision", f"{lr_metrics['Precision']:.4f}"])
    lr_table.add_row(["Recall",    f"{lr_metrics['Recall']:.4f}"])
    lr_table.add_row(["F1-score",  f"{lr_metrics['F1']:.4f}"])
    ...
    svm_info_table = PrettyTable()
    svm_info_table.field_names = ["Info", "Value"]
    svm_info_table.add_row(["Kernel",           "Linear (LinearSVC)"])
    svm_info_table.add_row(["TF-IDF train shape", str(x_train_tfidf.shape)])
    svm_info_table.add_row(["TF-IDF validation shape", str(x_validation_tfidf.shape)])
    print("\n=== SVM Info ===")
    print(svm_info_table)

    svm_model = LinearSVC(max_iter=1000)
    svm_model.fit(x_train_tfidf, y_train)
    y_pred_svm = svm_model.predict(x_validation_tfidf)

    svm_metrics = get_metrics(y_validation, y_pred_svm)
    svm_table = PrettyTable()
    svm_table.field_names = ["Metric", "Value"]
    svm_table.add_row(["Accuracy",  f"{svm_metrics['Accuracy']:.4f}"])
    svm_table.add_row(["Precision", f"{svm_metrics['Precision']:.4f}"])
    svm_table.add_row(["Recall",    f"{svm_metrics['Recall']:.4f}"])
    svm_table.add_row(["F1-score",  f"{svm_metrics['F1']:.4f}"])
    ...
    results.append(("Logistic Regression (TF-IDF)", lr_metrics))

    results.append(("SVM - LinearSVC (TF-IDF)", svm_metrics))
    ...
    ffnn_metrics = get_metrics(all_true, all_preds)
    ffnn_table = PrettyTable()
    ffnn_table.field_names = ["Metric", "Value"]
    ffnn_table.add_row(["Accuracy",  f"{ffnn_metrics['Accuracy']:.4f}"])
    ffnn_table.add_row(["Precision", f"{ffnn_metrics['Precision']:.4f}"])
    ffnn_table.add_row(["Recall",    f"{ffnn_metrics['Recall']:.4f}"])
    ffnn_table.add_row(["F1-score",  f"{ffnn_metrics['F1']:.4f}"])
    print("\n=== PyTorch FFNN Results ===")
    print(ffnn_table)

    # Add PyTorch results to your summary table
    results.append(("PyTorch FFNN (TF-IDF)", ffnn_metrics))
